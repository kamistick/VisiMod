...
#' 
#' Predictor values are extracted both locally (i.e., representing the terrain/vegetation characteristic of the pixel within which each point lies) and focally (i.e., within a focal neighborhood of pixels surrounding the point). In the case of focally derived predictors, the focal mean and standard deviation are calculated for each landscape variable within a neighborhood with radius `x` pixels for `x %in% c(2, 4, 6, 8, 16, 32)`. 
#' 
#' If the `vi_type == 'omnidir'`, the focal neighborhood will be a circle surrounding each point. If the `vi_type == 'directional_single' | vi_type == 'directional_random'`, the focal neighborhood will be a 'wedge' defined by the central viewing azimuth (`vi_azi`) and the angular field of view (`vi_fov`).
#' 
#' @details
#' * This is the fourth suggested function in the VisiMod workflow, following (1) [VisiMod::prep_dems()], (2) [VisiMod::gen_pts()], and (3) [VisiMod::calc_vi()]. 
